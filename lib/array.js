...
const { get, normalize } = require('./utils.js')

/**
 * Tests whether an array or string includes a value or not,
 * and returns either true or false
 *
 * @example
 * arrayOrStringIncludes(['Orange', 'Banana', 'Apple'], 'Orange')
 * // true
 * @param {(Array|string)} array - Array or string to search
 * @param {object} searchElement - value to search for
...
/**
 * Convert array into a string formatted as a sentence
 *
 * @example
 ...
 * @param {Intl.ListFormatType} [type] - Format of output
...
/**
 * Check value is classified as an `Array` object
 *
 * @example
...
function rejectFromArray(array, key, value) {
  return array.filter((item) => ![value].includes(get(item, key)))
}
...
views.addFilter('includes', arrayOrStringIncludes)
